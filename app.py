import streamlit as st

# ページ設定
st.set_page_config(page_title="花合わせ計算機", page_icon="🎴")

st.title("🎴 花合わせ計算機")
st.write("誰がどの札を持っているか選んで、最後に「計算する」を押してください。")

# --- 1. カードデータの定義 (カタカナ表記) ---
# 月, 種類, 点数, 表示名, ID
card_data = [
    # 1月 マツ
    ("1月", "光", 20, "マツのヒカリ(ツル)", "matsu_hikari"),
    ("1月", "短冊", 5, "マツのアカタン", "matsu_tan"),
    ("1月", "カス", 1, "マツのカス(1)", "matsu_kasu_1"),
    ("1月", "カス", 1, "マツのカス(2)", "matsu_kasu_2"),
    # 2月 ウメ
    ("2月", "タネ", 10, "ウメのタネ(ウグイス)", "ume_tane"),
    ("2月", "短冊", 5, "ウメのアカタン", "ume_tan"),
    ("2月", "カス", 1, "ウメのカス(1)", "ume_kasu_1"),
    ("2月", "カス", 1, "ウメのカス(2)", "ume_kasu_2"),
    # 3月 サクラ
    ("3月", "光", 20, "サクラのヒカリ(マク)", "sakura_hikari"),
    ("3月", "短冊", 5, "サクラのアカタン", "sakura_tan"),
    ("3月", "カス", 1, "サクラのカス(1)", "sakura_kasu_1"),
    ("3月", "カス", 1, "サクラのカス(2)", "sakura_kasu_2"),
    # 4月 フジ
    ("4月", "タネ", 10, "フジのタネ(ホトトギス)", "fuji_tane"),
    ("4月", "短冊", 5, "フジのタンザク", "fuji_tan"),
    ("4月", "カス", 1, "フジのカス(1)", "fuji_kasu_1"),
    ("4月", "カス", 1, "フジのカス(2)", "fuji_kasu_2"),
    # 5月 アヤメ
    ("5月", "タネ", 10, "アヤメのタネ(ヤツハシ)", "ayame_tane"),
    ("5月", "短冊", 5, "アヤメのアオタン", "ayame_tan"),
    ("5月", "カス", 1, "アヤメのカス(1)", "ayame_kasu_1"),
    ("5月", "カス", 1, "アヤメのカス(2)", "ayame_kasu_2"),
    # 6月 ボタン
    ("6月", "タネ", 10, "ボタンのタネ(チョウ)", "botan_tane"),
    ("6月", "短冊", 5, "ボタンのアオタン", "botan_tan"),
    ("6月", "カス", 1, "ボタンのカス(1)", "botan_kasu_1"),
    ("6月", "カス", 1, "ボタンのカス(2)", "botan_kasu_2"),
    # 7月 ハギ
    ("7月", "タネ", 10, "ハギのタネ(イノシシ)", "hagi_tane"),
    ("7月", "短冊", 5, "ハギのタンザク", "hagi_tan"),
    ("7月", "カス", 1, "ハギのカス(1)", "hagi_kasu_1"),
    ("7月", "カス", 1, "ハギのカス(2)", "hagi_kasu_2"),
    # 8月 ススキ
    ("8月", "光", 20, "ススキのヒカリ(ツキ)", "susuki_hikari"),
    ("8月", "タネ", 10, "ススキのタネ(カリ)", "susuki_tane"),
    ("8月", "カス", 1, "ススキのカス(1)", "susuki_kasu_1"),
    ("8月", "カス", 1, "ススキのカス(2)", "susuki_kasu_2"),
    # 9月 キク
    ("9月", "タネ", 10, "キクのタネ(サカズキ)", "kiku_tane"),
    ("9月", "短冊", 5, "キクのアオタン", "kiku_tan"),
    ("9月", "カス", 1, "キクのカス(1)", "kiku_kasu_1"),
    ("9月", "カス", 1, "キクのカス(2)", "kiku_kasu_2"),
    # 10月 モミジ
    ("10月", "タネ", 10, "モミジのタネ(シカ)", "momiji_tane"),
    ("10月", "短冊", 5, "モミジのアオタン", "momiji_tan"),
    ("10月", "カス", 1, "モミジのカス(1)", "momiji_kasu_1"),
    ("10月", "カス", 1, "モミジのカス(2)", "momiji_kasu_2"),
    # 11月 ヤナギ
    ("11月", "光", 20, "ヤナギのヒカリ(オノノトウフウ)", "yanagi_hikari"),
    ("11月", "タネ", 10, "ヤナギのタネ(ツバメ)", "yanagi_tane"),
    ("11月", "短冊", 5, "ヤナギのタンザク", "yanagi_tan"),
    ("11月", "カス", 1, "ヤナギのカス(カミナリ)", "yanagi_kasu"),
    # 12月 キリ
    ("12月", "光", 20, "キリのヒカリ(ホウオウ)", "kiri_hikari"),
    ("12月", "カス", 1, "キリのカス(1)", "kiri_kasu_1"),
    ("12月", "カス", 1, "キリのカス(2)", "kiri_kasu_2"),
    ("12月", "カス", 1, "キリのカス(3)", "kiri_kasu_3"),
]

# --- 2. 役判定ロジック (任天堂 花合わせルール) ---
def calculate_score(cards):
    points = sum(c[2] for c in cards)
    card_ids = set(c[4] for c in cards)
    yaku_list = []
    yaku_score = 0
    
    # 判定用セット
    hikari_ids = {c[4] for c in cards if c[1] == "光"}
    tan_ids = {c[4] for c in cards if c[1] == "短冊"}
    
    # --- 役判定 ---
    # 五光 (200点)
    if len(hikari_ids) == 5:
        yaku_list.append("ゴコウ(200)")
        yaku_score += 200
    # 四光 (60点)
    elif len(hikari_ids) == 4:
        yaku_list.append("シコウ(60)")
        yaku_score += 60
        
    # 七短 (40点)
    if len(tan_ids) >= 7:
        yaku_list.append("ナナタン(40)")
        yaku_score += 40
    elif len(tan_ids) == 6:
        yaku_list.append("ロクタン(30)")
        yaku_score += 30

    # 赤短 (40点)
    akatan_set = {"matsu_tan", "ume_tan", "sakura_tan"}
    if akatan_set.issubset(card_ids):
        yaku_list.append("アカタン(40)")
        yaku_score += 40
        
    # 青短 (40点)
    aotan_set = {"botan_tan", "kiku_tan", "momiji_tan"}
    if aotan_set.issubset(card_ids):
        yaku_list.append("アオタン(40)")
        yaku_score += 40
        
    # 猪鹿蝶 (20点)
    inoshikacho_set = {"hagi_tane", "momiji_tane", "botan_tane"}
    if inoshikacho_set.issubset(card_ids):
        yaku_list.append("イノシカチョウ(20)")
        yaku_score += 20
        
    # 花見で一杯 (20点)
    hanami_set = {"sakura_hikari", "kiku_tane"}
    if hanami_set.issubset(card_ids):
        yaku_list.append("ハナミデイッパイ(20)")
        yaku_score += 20
        
    # 月見で一杯 (20点)
    tsukimi_set = {"susuki_hikari", "kiku_tane"}
    if tsukimi_set.issubset(card_ids):
        yaku_list.append("ツキミデイッパイ(20)")
        yaku_score += 20

    return points, yaku_score, yaku_list

# --- 3. UI構築 ---
# 入力データの保持用辞書
user_selections = {}

# 月ごとにグルーピングして表示
current_month = ""
for month, type_, point, name, id_ in card_data:
    if month != current_month:
        st.markdown(f"### {month}")
        current_month = month
    
    # 選択ラジオボタン（横並び）
    # keyをユニークにする必要がある
    user_selections[id_] = st.radio(
        label=f"{name}",
        options=["未", "A", "B", "C"],
        horizontal=True,
        key=id_
    )
    st.markdown("---") # 区切り線

# --- 4. 計算実行 ---
if st.button("計算する", type="primary"):
    # 1. 選択情報の収集
    p_cards = {'A': [], 'B': [], 'C': []}
    
    # 全カードデータを走査して振り分け
    for month, type_, point, name, id_ in card_data:
        owner = user_selections[id_]
        if owner in p_cards:
            p_cards[owner].append((month, type_, point, name, id_))
    
    # 2. 計算
    results = {}
    total_yaku_score = 0
    
    # 各プレイヤーの基礎計算
    for p_name in ['A', 'B', 'C']:
        cards = p_cards[p_name]
        fuda_ten, yaku_ten, yakus = calculate_score(cards)
        results[p_name] = {
            'fuda': fuda_ten,
            'yaku': yaku_ten,
            'yakus': yakus
        }
        total_yaku_score += yaku_ten
        
    # 3. 結果表示
    st.header("🎴 集計結果")
    grand_total = 0
    
    cols = st.columns(3) # 3列で表示
    
    for i, p_name in enumerate(['A', 'B', 'C']):
        r = results[p_name]
        others_yaku = total_yaku_score - r['yaku']
        final_score = (r['fuda'] - 88) + (r['yaku'] * 2) - others_yaku
        grand_total += final_score
        
        with cols[i]:
            st.subheader(f"プレイヤー {p_name}")
            st.metric(label="最終得点", value=f"{final_score:+}点")
            
            st.markdown(f"""
            **詳細:**
            - 獲得枚数: {len(p_cards[p_name])}枚
            - 札点: {r['fuda']}点
            - 役: {', '.join(r['yakus']) if r['yakus'] else 'ナシ'}
            """)

    # 検算
    st.markdown("---")
    if grand_total == 0:
        st.success(f"✅ 計算完了！ 合計はピッタリ 0点 です。")
    else:
        st.error(f"⚠️ エラー: 合計が {grand_total}点 になっています。すべての札を選択しましたか？")
